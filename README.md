...
📫 可用代理：741 个 (✅ 698 存活 · ⚠️ 43 缓冲 [1次: 35 · 2次: 8] · ⚡ 均延 520ms)
🌐 单条优选：37 条 (✅ 28 存活 · ⚠️ 9 缓冲 [1次: 7 · 2次: 2])
📁 扫描优选：5,015 条 (✅ 4,862 存活 · ⚠️ 153 缓冲 [1次: 110 · 2次: 43] · 20 个 ASN)
   └ 涵盖: Aeza, DMIT, ByteVirt, Starry Network 等
🔀 反代 ProxyIP：30,545 条 (✅ 29,820 存活 · ⚠️ 725 缓冲 [1次: 450 · 2次: 275])
...
* **增量与健康一览（核心：什么是“存活”与“缓冲细分”？）**：
  - **✅ 存活（`fail_count = 0`）**：在当次 CI 流水线中成功通过真实握手/穿透质检的节点，100% 验证为高可用健康节点。
  - **⚠️ 缓冲细分（`1 <= fail_count < 3`）**：在当次探测中因公网跨洲抖动、偶发丢包或目标端短暂限流导致超时的节点。系统设定了 **1~2 次容忍缓冲期**（保留在库中不立即物理删除），并**在卡片中精准展开 1 次与 2 次的具体数量**：
    - **`1次: N`（偶发抖动）**：本次刚出现超时的节点，多为跨洲骨干网瞬时波动，自愈恢复概率极高。
    - **`2次: N`（濒危预警）**：已连续 2 次探测不可达的节点，处于淘汰边缘（若下次仍然失败累积满 3 次，将立即触发物理剔除与墓地冷却）。
    - **自愈复活**：任何缓冲节点在下一次质检中只要重新连通，`fail_count` 立即清零，无缝复活回到“✅ 存活”状态。
