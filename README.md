...
### 7. 优选 IP 两阶段主动鉴真（`cf_verify.py`）
为防止长期累积的节点死灰或被封禁失效，系统配备异步高并发探测机制：
...
* **淘汰保护机制**：
  * 校验通过：更新实测延迟 `delay_ms`，并将 `fail_count` 重置为 `0`。
  * 校验失败：`fail_count` 累加 `1`，仅当连续失败达到阈值（默认 3 次）时才永久淘汰剔除，杜绝因临时网络抖动误杀优质节点。
  * 未开启 TLS 的节点（`tls: false`）：自动跳过 TLS 探测，保留原样。
* **工作流联动**：GitHub Actions 每日定时运行抓取后自动执行校验，保持优选池高度纯净。
