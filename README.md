...
* **增量与健康一览（核心：什么是“存活”与“缓冲”？）**：
  - **✅ 存活（`fail_count = 0`）**：在当次 CI 流水线中成功通过真实握手/穿透质检的节点，100% 验证为高可用健康节点。
  - **⚠️ 缓冲（`1 <= fail_count < 3`）**：在当次探测中因公网跨洲抖动、偶发丢包或目标端短暂限流导致超时的节点。系统设定了 **1~2 次容忍缓冲期**（保留在库中不立即物理删除），避免因单次网络偶然波动“误杀”高价值节点：
    - **自愈复活**：若在下一次日常质检中网络恢复连通，`fail_count` 立即清零，无缝复活回到“✅ 存活”状态。
    - **超时淘汰**：若连续多次质检均不可达，累积达到 3 次（`fail_count >= 3`），系统才判定其为不可逆的真正死节点，彻底物理删除并写入墓地。
